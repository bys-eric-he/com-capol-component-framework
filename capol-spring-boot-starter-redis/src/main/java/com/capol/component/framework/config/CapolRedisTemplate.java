 ...
 */
@Slf4j
public class CapolRedisTemplate {
    ...
    private RedisAdapter getRedisAdapter() {
        ...
        if (!environment.getRedisConnectionFactoryIsInit()) {
            log.error("Please open the redis integration annotation @EnableRedis and select redis mode {single, cluster, sentinel}");
            throw new RuntimeException("Please open the redis integration annotation @EnableRedis and select redis mode {single, cluster, sentinel}");
        }
